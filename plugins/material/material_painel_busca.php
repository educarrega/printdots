...
@import url("plugins/material/material.css?uid=<?= $uid ?>");
</style>

<script type="text/javascript">

function lookup_material(inputString) {
$('#bt_limpar_material').show();
if(inputString.length == 0) {
	$('#bt_limpar_material').hide();
	...
	busca_status("<img src='images/loader.gif' class='loader'> Efetuando sua busca por: "+inputString);
    	$('#material_todas').load("utf2iso.php?file=plugins/material/material_lista.php", {busca: ""+ encodeURI(inputString) +""});
}
} // lookup

$('#bt_limpar_material').click(function(){
	$('#inputString').val('');
	lookup_material('');
});
...
<div id="producao_painel_busca">
	...
	<div class="producao_painel_busca_right">
		...
		<input type="text" name="<?= $uid ?>" id="inputString" onkeyup="lookup_material(this.value);" />
		<input type="image" id="bt_limpar_material" src="images/bt_limpar.png?uid=<?= $uid ?>" title="Limpar a busca" />
		</div>
	</div>
</div>
