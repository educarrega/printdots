...
@import url("plugins/banco/banco.css?uid=<?= $uid ?>");
</style>

<script type="text/javascript">

function lookup_banco(inputString) {
$('#bt_limpar_banco').show();
if(inputString.length == 0) {
	$('#bt_limpar_banco').hide();
	...
	busca_status("<img src='images/loader.gif' class='loader'> Efetuando sua busca por: "+inputString);
    	$('#banco_todas').load("utf2iso.php?file=plugins/banco/banco_lista.php", {busca: ""+ encodeURI(inputString) +""});
}
} // lookup

$('#bt_limpar_banco').click(function(){
	$('#inputString').val('');
	lookup_banco('');
});
...
<div id="producao_painel_busca">
	...
	<div class="producao_painel_busca_right">
		...
		<input type="text" name="<?= $uid ?>" id="inputString" onkeyup="lookup_banco(this.value);" />
		<input type="image" id="bt_limpar_banco" src="images/bt_limpar.png?uid=<?= $uid ?>" title="Limpar a busca" />
		</div>
	</div>
</div>
