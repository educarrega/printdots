...
@import url("plugins/transporte/transporte.css?uid=<?= $uid ?>");
</style>

<script type="text/javascript">

function lookup_transporte(inputString) {
$('#bt_limpar_transporte').show();
if(inputString.length == 0) {
	$('#bt_limpar_transporte').hide();
	...
	busca_status("<img src='images/loader.gif' class='loader'> Efetuando sua busca por: "+inputString);
    	$('#transporte_todas').load("utf2iso.php?file=plugins/transporte/transporte_lista.php", {busca: ""+ encodeURI(inputString) +""});
}
} // lookup

$('#bt_limpar_transporte').click(function(){
	$('#inputString').val('');
	lookup_transporte('');
});
...
<div id="producao_painel_busca">
	...
	<div class="producao_painel_busca_right">
		...
		<input type="text" name="<?= $uid ?>" id="inputString" onkeyup="lookup_transporte(this.value);" />
		<input type="image" id="bt_limpar_transporte" src="images/bt_limpar.png?uid=<?= $uid ?>" title="Limpar a busca" />
		</div>
	</div>
</div>
